...
class VirtualTradingService {
    ...
    async executeOrder(userId, orderData) {
        try {
            ...
            const fees = totalAmount * this.tradingFees;
            ...
            if (type === 'BUY') {
                ...
                this.addToHoldings(portfolio, symbol, quantity, currentPrice, totalAmount);
            } else {
                portfolio.balance += (totalAmount - fees);
                this.removeFromHoldings(portfolio, symbol, quantity, currentPrice);
            }
    ...
    addToHoldings(portfolio, symbol, quantity, price, totalAmount) {
    ...
    removeFromHoldings(portfolio, symbol, quantity, price) {
        const holding = portfolio.holdings.find(h => h.symbol === symbol);
    ...
    async getPortfolioSummary(userId) {
        try {
            const portfolio = await this.updatePortfolioValues(userId);
            
            if (!portfolio) {
                // Create new portfolio if it doesn't exist
                const newPortfolio = await this.getOrCreatePortfolio(userId);
                return {
...
export default new VirtualTradingService();